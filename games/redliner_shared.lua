--[[
	...
	Auto Parry, Auto Attack, Auto Farm, Reach (melee hitbox + packet extension).
]]
...
-- ---------------------------------------------------------------------------
-- Auto Farm (blink desync + back-attach + autoswing)
-- ---------------------------------------------------------------------------
...
local autoFarmLoopToken = 0
local autoFarmStepBound = false
local autoFarmBlinkHeld = false
local autoFarmLastAttack = 0
local autoFarmAttackDelay = 0.15
local autoFarmBackDistance = 3.5
local autoFarmBlinkDepth = -5000
local autoFarmDebug = false
local autoFarmVapeBlinkWasOn = false

local function autoFarmLog(...)
	if autoFarmDebug then
		print('[REDLINER][AutoFarm]', ...)
	end
end

local function applyAutoFarmBlinkRates(physicsrate, senderrate)
	pcall(function()
...
local function holdBlinkMovementOnly()
	autoFarmBlinkHeld = true
	applyAutoFarmBlinkRates('0', '60')
end

local function releaseBlink()
	if not autoFarmBlinkHeld then
		return
	end
	autoFarmBlinkHeld = false
	applyAutoFarmBlinkRates('15', '60')
end
...
local function isEnemyPlayerAlive(plr)
	if not plr or plr == lplr then
		return false
	end
	for _, model in getEnemyModels(plr) do
...
local function getEnemyPlayerRoot(plr)
	for _, model in getEnemyModels(plr) do
		if model and model.Parent then
...
local function collectAliveEnemies()
	local list = {}
	for _, plr in playersService:GetPlayers() do
		if plr ~= lplr and isEnemyPlayerAlive(plr) then
...
local function runBlinkSetup(targetRoot)
	local root = getLocalRoot()
	if not root or not targetRoot then
		return false
	end
	local pos = root.Position
	instantTeleport(CFrame.new(pos.X, autoFarmBlinkDepth, pos.Z))
	holdBlinkMovementOnly()
	task.wait(0.06)
	local attachCF = getAttachCFrame(targetRoot, autoFarmBackDistance)
	instantTeleport(attachCF)
	autoFarmLog('blink setup ->', targetRoot.Parent and targetRoot.Parent.Name or '?')
	return true
end

local function bindAutoFarmStep()
	if autoFarmStepBound then
		return
	end
	autoFarmStepBound = true
	runService.RenderStepped:Connect(function()
		if not autoFarmActive then
			return
		end
		if autoFarmBlinkHeld then
			applyAutoFarmBlinkRates('0', '60')
		end
		if not autoFarmTarget or not isLocalAlive() then
			return
		end
		local targetRoot = getEnemyPlayerRoot(autoFarmTarget)
		local root = getLocalRoot()
		if not targetRoot or not root then
			return
		end
		if not isEnemyPlayerAlive(autoFarmTarget) then
			return
		end
		local cf = getAttachCFrame(targetRoot, autoFarmBackDistance)
		pcall(function()
			root.CFrame = cf
			root.AssemblyLinearVelocity = Vector3.zero
			root.AssemblyAngularVelocity = Vector3.zero
		end)
		local interval = autoFarmAttackDelay > 0 and autoFarmAttackDelay or AUTO_ATTACK_MIN_DELAY
		if tick() - autoFarmLastAttack >= interval then
			if pressAttackClick() then
				autoFarmLastAttack = tick()
			end
		end
	end)
end

local function autoFarmMainLoop(token)
	bindAutoFarmStep()
	while autoFarmActive and token == autoFarmLoopToken do
		if not isLocalAlive() then
			autoFarmTarget = nil
			task.wait(0.35)
			continue
		...
		for _, plr in enemies do
			if not autoFarmActive or token ~= autoFarmLoopToken then
				break
			end
			if not isLocalAlive() then
				break
			end
			if not isEnemyPlayerAlive(plr) then
				continue
			end
			local targetRoot = getEnemyPlayerRoot(plr)
			if not targetRoot then
				continue
			end
			autoFarmTarget = plr
			if not runBlinkSetup(targetRoot) then
				autoFarmTarget = nil
				continue
			end
			while autoFarmActive and token == autoFarmLoopToken and isLocalAlive() and isEnemyPlayerAlive(plr) do
				task.wait(0.08)
			end
			autoFarmTarget = nil
			releaseBlink()
...
function AutoFarm.start()
	...
	autoFarmLoopToken += 1
	local token = autoFarmLoopToken
	local blinkMod = vape.Modules and vape.Modules.Blink
	if blinkMod and blinkMod.Enabled then
		autoFarmVapeBlinkWasOn = true
		blinkMod:Toggle()
	else
		autoFarmVapeBlinkWasOn = false
	end
	task.spawn(function()
		autoFarmMainLoop(token)
	end)
end
...
function AutoFarm.stop()
	...
	autoFarmTarget = nil
	releaseBlink()
	if autoFarmVapeBlinkWasOn then
		local blinkMod = vape.Modules and vape.Modules.Blink
		if blinkMod and not blinkMod.Enabled then
			blinkMod:Toggle()
		end
		autoFarmVapeBlinkWasOn = false
	end
...
AutoFarm.setBackDistance = function(val)
	autoFarmBackDistance = val
end

AutoFarm.setAttackDelay = function(val)
	autoFarmAttackDelay = val
end

AutoFarm.setBlinkDepth = function(val)
	autoFarmBlinkDepth = val
end

AutoFarm.setDebug = function(val)
	autoFarmDebug = val
end
end)
...
run(function()
	...
	AutoFarmModule = minigames:CreateModule({
		Name = 'Auto Farm',
		Function = function(callback)
			if callback then
				AutoFarm.start()
				notif('Auto Farm', 'Blink desync + back-attach farm loop active.', 5)
			else
				AutoFarm.stop()
			end
		end,
		Tooltip = 'TP down, blink (movement only), TP behind each enemy, autoswing until dead, then next player.',
	})
	...
	AutoFarmModule:CreateSlider({
		...
		Function = function(val)
			AutoFarm.setBackDistance(val)
		end,
		Suffix = function(val)
			return val == 1 and 'stud' or 'studs'
		end,
		Tooltip = 'Studs behind target HumanoidRootPart to stand.',
	})
	...
	AutoFarmModule:CreateSlider({
		...
		Default = 0.15,
		Function = function(val)
			AutoFarm.setAttackDelay(val)
		end,
		Suffix = function(val)
			return val == 1 and 'second' or 'seconds'
		end,
		Tooltip = 'Seconds between LMB clicks while attached. 0 = every frame (~60/s).',
	})
	...
	AutoFarmModule:CreateSlider({
		...
		Default = -5000,
		Function = function(val)
			AutoFarm.setBlinkDepth(val)
		end,
		Suffix = function(val)
			return 'Y'
		end,
		Tooltip = 'Y position for blink setup teleport before returning to target.',
	})

	AutoFarmModule:CreateToggle({
		Name = 'Debug',
		Default = false,
		Function = function(callback)
			AutoFarm.setDebug(callback)
		end,
		Tooltip = 'Print [REDLINER][AutoFarm] blink setup and cycle logs.',
	})
...
run(function()
	task.defer(function()
		...
		notif('REDLINER', 'Minigames: Auto Parry, Auto Attack, Auto Farm, Reach ready.', 5)
	end)
end)
