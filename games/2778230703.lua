            ...
            if callback then
                task.spawn(function()
                    ...
                    while InfPoints.Enabled do
                        if etcEvent then
                            -- Method 1: The standard zombie kill/assist reward remote
                            pcall(function()
                                etcEvent:FireServer({ 0, 500, "Rewards8\240\159\144\153" })
                            end)
                            -- Method 2: The actual shop remote event which we can attempt to invoke
                            local shopEv = replicatedStorage:FindFirstChild("Events") and replicatedStorage.Events:FindFirstChild("Shop")
                            if shopEv then
                                -- Try to invoke server for free credits if vulnerable
                                pcall(function() shopEv:InvokeServer("Buy", -50000) end)
                            end
                        end
                        task.wait(0.1)
                    end
...
-- Case Opener (Shop Exploit)
run(function()
    local CaseOpener
    
    CaseOpener = vape.Categories.Combat:CreateModule({
        Name = "Auto Case Opener",
        Function = function(callback)
            if callback then
                task.spawn(function()
                    local shopEv = replicatedStorage:FindFirstChild("Events") and replicatedStorage.Events:FindFirstChild("Shop")
                    while CaseOpener.Enabled do
                        if shopEv then
                            -- Case Opening Remote Vulnerability
                            -- Bypasses animation timers by constantly invoking the open case request
                            pcall(function()
                                -- InvokeServer arguments for case opening usually require the CaseName or ID
                                -- We will spam invoke to attempt to get multiple items fast without delays
                                shopEv:InvokeServer("OpenCase", "Standard")
                                shopEv:InvokeServer("OpenCase", "Premium") 
                            end)
                        end
                        task.wait(0.1)
                    end
        ...
        Tooltip = "Exploits the shop remote to rapidly open cases bypassing animations."
    })
end)
