...
for _, v3 in v2 do
    if u1.DevProductEntitlements[v3.product_id] then
        error((`Duplicate ProductId in receipt entitlement registry: {v3.name} ({v3.product_id})`));
    end;
    u1.DevProductEntitlements[v3.product_id] = {
        name = v3.name,
        fulfillment_kind = v3.fulfillment_kind,
        fulfillment_data = v3.fulfillment_data,
        transferable = v3.transferable
    };
end;
u1.MessageTypes = {
    MatchLeave = 1,
...
function u1.GetPingData(p10) --[[ Line: 309 ]]
    -- upvalues: u4 (copy), u1 (copy)
    for _, v11 in u4 do
        if p10 <= v11.data.threshold then
            return v11.key, v11.data;
        end;
    end;
    return "Bad", u1.PingData.Bad;
end;
function u1.reverse(p12, p13) --[[ Line: 321 ]]
    -- upvalues: u5 (copy)
    local v14 = u5[p12];
    if not v14 then
