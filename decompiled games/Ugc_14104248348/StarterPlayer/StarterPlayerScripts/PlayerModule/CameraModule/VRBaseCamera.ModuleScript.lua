-- Decompiled from: StarterPlayer.StarterPlayerScripts.PlayerModule.CameraModule.VRBaseCamera
-- Class: ModuleScript
...
require(l__CommonUtils__8:WaitForChild("FlagUtil"));
local l__BaseCamera__9 = require(script.Parent:WaitForChild("BaseCamera"));
local u4 = setmetatable({}, l__BaseCamera__9);
u4.__index = u4;
function u4.new() --[[ Line: 40 ]]
    -- upvalues: l__BaseCamera__9 (copy), u4 (copy)
    local v5 = l__BaseCamera__9.new();
...
function u4.Reset(p7) --[[ Line: 66 ]]
    p7.stepRotateTimeout = 0;
end;
function u4.GetModuleName(_) --[[ Line: 70 ]]
    return "VRBaseCamera";
end;
function u4.GamepadZoomPress(p8) --[[ Line: 74 ]]
    -- upvalues: l__BaseCamera__9 (copy)
    l__BaseCamera__9.GamepadZoomPress(p8);
    p8:GamepadReset();
    p8:ResetZoom();
end;
function u4.GamepadReset(p9) --[[ Line: 82 ]]
    p9.stepRotateTimeout = 0;
    p9.needsReset = true;
end;
function u4.ResetZoom(p10) --[[ Line: 87 ]]
    -- upvalues: l__ZoomController__7 (copy)
    l__ZoomController__7.SetZoomParameters(p10.currentSubjectDistance, 0);
    l__ZoomController__7.ReleaseSpring();
end;
function u4.OnEnabledChanged(u11) --[[ Line: 92 ]]
    ...
    l__BaseCamera__9.OnEnabledChanged(u11);
    if u11.enabled then
        u11.gamepadResetConnection = l__CameraInput__6.gamepadReset:Connect(function() --[[ Line: 96 ]]
            -- upvalues: u11 (copy)
            u11:GamepadReset();
        end);
        u11.thirdPersonOptionChanged = l__VRService__1:GetPropertyChangedSignal("ThirdPersonFollowCamEnabled"):Connect(function() --[[ Line: 101 ]]
        ...
        u11.vrRecentered = l__VRService__1.UserCFrameChanged:Connect(function(p12, _) --[[ Line: 112 ]]
            -- upvalues: u11 (copy)
        ...
        u11:UpdateEdgeBlur(l__LocalPlayer__2, 1);
        local v13 = l__Lighting__3:FindFirstChild("VRFade");
        if v13 then
            v13.Brightness = 0;
        end;
    end;
end;
function u4.OnCurrentCameraChanged(u14) --[[ Line: 154 ]]
    -- upvalues: l__BaseCamera__9 (copy)
    l__BaseCamera__9.OnCurrentCameraChanged(u14);
    ...
    if l__CurrentCamera__10 then
        u14.cameraHeadScaleChangedConn = l__CurrentCamera__10:GetPropertyChangedSignal("HeadScale"):Connect(function() --[[ Line: 166 ]]
            -- upvalues: u14 (copy)
...
function u4.OnHeadScaleChanged(p15) --[[ Line: 171 ]]
    local l__HeadScale__11 = workspace.CurrentCamera.HeadScale;
    for v16, v17 in p15.gamepadZoomLevels do
        p15.gamepadZoomLevels[v16] = v17 * l__HeadScale__11 / p15.headScale;
    end;
    p15:SetCameraToSubjectDistance(p15:GetCameraToSubjectDistance() * l__HeadScale__11 / p15.headScale);
    p15.headScale = l__HeadScale__11;
end;
function u4.GetVRFocus(p18, p19, p20) --[[ Line: 187 ]]
    local v21 = p18.lastCameraFocus or p19;
    local l__x__12 = p18.cameraTranslationConstraints.x;
    local v22 = math.min(1, p18.cameraTranslationConstraints.y + p20);
    p18.cameraTranslationConstraints = Vector3.new(l__x__12, v22, p18.cameraTranslationConstraints.z);
...
function u4.StartFadeFromBlack(p25) --[[ Line: 203 ]]
    -- upvalues: l__UserGameSettings__5 (copy), l__Lighting__3 (copy)
    if l__UserGameSettings__5.VignetteEnabled == false then
    else
        local v26 = l__Lighting__3:FindFirstChild("VRFade");
        if not v26 then
            v26 = Instance.new("ColorCorrectionEffect");
            v26.Name = "VRFade";
            v26.Parent = l__Lighting__3;
        end;
        v26.Brightness = -1;
        p25.VRFadeResetTimer = 0.1;
    end;
end;
function u4.UpdateFadeFromBlack(p27, p28) --[[ Line: 218 ]]
    -- upvalues: l__Lighting__3 (copy)
    local v29 = l__Lighting__3:FindFirstChild("VRFade");
...
function u4.StartVREdgeBlur(p31, p32) --[[ Line: 234 ]]
    ...
    if l__UserGameSettings__5.VignetteEnabled == false then
        ...
        if not v33 then
            ...
            l__RunService__4.RenderStepped:Connect(function(_) --[[ Line: 254 ]]
...
function u4.UpdateEdgeBlur(p40, p41, p42) --[[ Line: 303 ]]
...
function u4.GetCameraHeight(p47) --[[ Line: 328 ]]
    return p47.inFirstPerson and 0 or 0.25881904510252074 * p47.currentSubjectDistance;
end;
function u4.GetSubjectCFrame(p48) --[[ Line: 335 ]]
    -- upvalues: l__BaseCamera__9 (copy)
    local v49 = l__BaseCamera__9.GetSubjectCFrame(p48);
    ...
    if l__CurrentCamera__13:IsA("Humanoid") and (l__CurrentCamera__13:GetState() == Enum.HumanoidStateType.Dead and l__CurrentCamera__13 == p48.lastSubject) then
...
function u4.GetSubjectPosition(p50) --[[ Line: 361 ]]
    -- upvalues: l__BaseCamera__9 (copy)
    local v51 = l__BaseCamera__9.GetSubjectPosition(p50);
    ...
    if l__CurrentCamera__14:IsA("Humanoid") then
        if l__CurrentCamera__14:GetState() == Enum.HumanoidStateType.Dead and l__CurrentCamera__14 == p50.lastSubject then
            v51 = p50.lastSubjectPosition;
        end;
    elseif l__CurrentCamera__14:IsA("VehicleSeat") then
        v51 = l__CurrentCamera__14.CFrame.p + l__CurrentCamera__14.CFrame:vectorToWorldSpace(Vector3.new(0, 4, 0));
    end;
    p50.lastSubjectPosition = v51;
    return v51;
end;
function u4.getRotation(p52, p53) --[[ Line: 390 ]]
    -- upvalues: l__CameraInput__6 (copy), l__UserGameSettings__5 (copy)
    local v54 = l__CameraInput__6.getRotation(p53);
    local v55 = 0;
