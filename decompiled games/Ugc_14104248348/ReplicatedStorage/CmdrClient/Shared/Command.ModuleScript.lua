-- Decompiled from: ReplicatedStorage.CmdrClient.Shared.Command
-- Class: ModuleScript
...
function u2.Run(p26) --[[ Line: 117 ]]
    ...
    if p26.Response == nil then
        ...
        elseif u1 then
            if p26.Object.ClientRun then
                warn(p26.Name, "command fell back to the server because ClientRun returned nil, but there is no server implementation! Either return a string from ClientRun, or create a server implementation for this command.");
            else
                warn(p26.Name, "command has no implementation!");
            end;
...
function u2.GetArgument(p34, p35) --[[ Line: 164 ]]
    return p34.Arguments[p35];
end;
function u2.GetData(p36) --[[ Line: 172 ]]
    -- upvalues: u1 (copy)
    if p36.Data then
        return p36.Data;
    end;
...
function u2.SendEvent(p37, p38, p39, ...) --[[ Line: 185 ]]
...
function u2.BroadcastEvent(p43, ...) --[[ Line: 199 ]]
    -- upvalues: u1 (copy)
    if not u1 then
...
function u2.Reply(p44, ...) --[[ Line: 208 ]]
    return p44:SendEvent(p44.Executor, "AddLine", ...);
end;
function u2.GetStore(p45, ...) --[[ Line: 213 ]]
    return p45.Dispatcher.Cmdr.Registry:GetStore(...);
end;
function u2.HasImplementation(p46) --[[ Line: 218 ]]
