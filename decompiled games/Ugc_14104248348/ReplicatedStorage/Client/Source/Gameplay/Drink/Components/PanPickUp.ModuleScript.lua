-- Decompiled from: ReplicatedStorage.Client.Source.Gameplay.Drink.Components.PanPickUp
-- Class: ModuleScript
...
local l__Slice__4 = l__Mince__3.Config.Recipes.Slice;
local l__Pan__5 = l__Mince__3.Config.Machines.Pan;
local u5 = TweenInfo.new(0.5);
local v6 = l__Mince__3.Component({
    Tag = "PickupPan"
});
function v6.Removing(p7) --[[ Line: 29 ]]
    -- upvalues: u4 (copy)
...
function v6.Construct(u10) --[[ Line: 35 ]]
    -- upvalues: u3 (copy), l__Pan__5 (copy), l__Slice__4 (copy), u2 (copy), l__TweenService__2 (copy), u5 (copy), u1 (copy), u4 (copy)
    ...
    function u10.OnHover(p12) --[[ Line: 51 ]]
        -- upvalues: u3 (ref), l__Pan__5 (ref)
        local v13 = u3.GetEquippedTool();
        local v14 = p12.Instance:GetAttribute("DoughState");
        local v15 = p12.Instance:GetAttribute("Baked");
        if v13 then
            if v15 then
                if v13:GetAttribute("DrinkID") ~= "PizzaCutter" then
                    return false;
                end;
                p12.HoverName = `Cut {p12.DefaultName}  with the {v13.Name}`;
                return true;
            end;
            if not l__Pan__5.AllowedUses[v13:GetAttribute("DrinkID")] or v13:GetAttribute("Baked") then
                return false;
            end;
            local v16 = ({
                Dough = 2,
            ...
            if not (v16 or p12.Instance:GetAttribute("Sauce")) then
                return false;
            end;
    ...
    function u10.Interact(p17, p18) --[[ Line: 96 ]]
        -- upvalues: u3 (ref), l__Slice__4 (ref), u2 (ref), l__Pan__5 (ref), l__TweenService__2 (ref), u5 (ref), u1 (ref)
        local v19 = p17.Instance:GetAttribute("DoughState");
        local v20 = u3.GetEquippedTool();
        if v20 and v20:GetAttribute("DrinkID") == "PizzaCutter" then
            u3.DoProcessingText(l__Slice__4.Action, l__Slice__4.ActionTime, l__Slice__4.AltSound or "InteractProcess", true);
            u2:Fire(p17.Instance);
            p18();
        else
            if v20 and l__Pan__5.AllowedUses[v20:GetAttribute("DrinkID")] or v19 and v19 == 1 then
                if v19 == 1 then
                    local v21 = l__TweenService__2:Create(p17.Instance.Dough, u5, {
                        Transparency = 1,
                        Size = p17.Instance.Handle.Size
                    });
                    for _, v22 in p17.Instance.Slices:GetChildren() do
                        l__TweenService__2:Create(v22.Base, u5, {
                            Transparency = 0
                        }):Play();
                    end;
                    v21:Play();
    ...
    u10.Instance:GetPropertyChangedSignal("Name"):Connect(function() --[[ Line: 145 ]]
        -- upvalues: u10 (copy)
        u10.DefaultName = u10.Instance.Name;
    end);
